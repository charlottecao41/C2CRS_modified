...
from crslab.model.crs.c2crs.muli_type_data_module import CoarseFineKGModel
...
class C2CRS_System(BaseSystem):
    ...
    def train_kg(self):
        best_loss = 1e9
        self.rec_model.kGModel.to(self.device)
        ops =  torch.optim.Adam(self.rec_model.kGModel.parameters(),lr=1e-4)
        for epoch in range(self.pretrain_epoch):
            for b in self.train_dataloader['rec'].get_rec_data(self.pretrain_batch_size,shuffle=True):
                batch, unbatchify_batch = b
                for k, v in batch.items():
                    if isinstance(v, torch.Tensor):
                        batch[k] = v.to(self.device)
                ops.zero_grad()
                loss,score = self.rec_model.kGModel.recommend(batch,mode ='train')
                loss.backward()
                ops.step()
                torch.nn.utils.clip_grad_norm_(self.rec_model.kGModel.parameters(), 5)

            total_loss = 0
            with torch.no_grad():
                self.rec_model.kGModel.eval()
                for b in self.valid_dataloader['rec'].get_rec_data(self.pretrain_batch_size, 
                                                                    shuffle=False):
                    batch, unbatchify_batch = b
                    for k, v in batch.items():
                        if isinstance(v, torch.Tensor):
                            batch[k] = v.to(self.device)
                    loss,score = self.rec_model.kGModel.recommend(batch,mode ='valid')
                    total_loss=total_loss+loss.item()
            
            print('Valid Loss: ', total_loss)
            if total_loss < best_loss:
                best_loss = total_loss
                torch.save(self.rec_model.kGModel.state_dict(), f'save_model/best.pt')

        self.rec_model.kGModel.load_state_dict(torch.load('save_model/best.pt'))
        self.rec_model.kGModel.eval()
        with torch.no_grad():
            self.evaluator.reset_metrics()
            for b in self.test_dataloader['rec'].get_rec_data(self.rec_batch_size,
                                                                  shuffle=False):
                batch, unbatchify_batch = b
                for k, v in batch.items():
                    if isinstance(v, torch.Tensor):
                        batch[k] = v.to(self.device)
                loss,score = self.rec_model.kGModel.recommend(batch,mode ='test')
                self.rec_evaluate(score, batch['movie_to_rec'])
                rec_loss = loss.item()
                self.evaluator.optim_metrics.add("rec_loss",AverageMetric(rec_loss))
            self.evaluator.report()

        item_emb = self.rec_model.kGModel.get_all_item_rep()
        movie = {}
        entity = {}
        entity_link_to_id = json.load(open('data/dataset/redial/nltk/entity2id.json'))
        movie_link_to_id = pickle.load(open('movie_link_to_id.pkl','rb'))
        entity_id_to_link = {}
        for k in entity_link_to_id:
            entity_id_to_link.update({int(entity_link_to_id[k]):k})

        for i in range(1,64363):
            if entity_id_to_link[i] not in movie_link_to_id:
                entity.update({entity_id_to_link[i]:item_emb[i].cpu().detach().numpy()})
            else:
                movie_idx = movie_link_to_id[entity_id_to_link[i]]
                movie.update({movie_idx:item_emb[i].cpu().detach().numpy()})
        
        file = open('movie_emb.pkl', 'wb')
        pickle.dump(movie,file)
        file = open('entity_emb.pkl', 'wb')
